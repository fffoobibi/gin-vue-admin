<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="关联tbl_kol_resource_polices" prop="policyId" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.policyId) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateTblKolResourceCleanFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="关联tbl_kol_resource_polices">
                    {{ formatBoolean(formData.policyId) }}
                </el-descriptions-item>
...
import {
  ...
  deleteTblKolResourceClean,
  deleteTblKolResourceCleanByIds,
  updateTblKolResourceClean,
  findTblKolResourceClean,
  getTblKolResourceCleanList
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    if (searchInfo.value.policyId === ""){
        searchInfo.value.policyId=null
    }
...
const getTableData = async() => {
  const table = await getTblKolResourceCleanList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteTblKolResourceCleanFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteTblKolResourceCleanByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTblKolResourceCleanFunc = async(row) => {
    const res = await findTblKolResourceClean({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteTblKolResourceCleanFunc = async (row) => {
    const res = await deleteTblKolResourceClean({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
